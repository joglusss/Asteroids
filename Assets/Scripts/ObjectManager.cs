...
public class ObjectManager : MonoBehaviour, MenuScript.IResetable
{

    private static RectTransform s_CanvasRectTransformLink;
    public static Vector2 canvasSize { get { return s_CanvasRectTransformLink.sizeDelta * s_CanvasRectTransformLink.lossyScale; } }
    public static Vector2 speedScale { get { return s_CanvasRectTransformLink.lossyScale; } }

    public static bool IsOutCanvas(Vector2 position, out Vector2 wall) {
    ...
    public static void Teleport(Rigidbody2D rb, Vector2 wall)
    {


        if (wall == Vector2.left) {
            rb.position = TeleportPosition(Vector2.zero, Vector2.up * canvasSize.y, rb.position);
            return;
        }
        if (wall == Vector2.right) {
            rb.position = TeleportPosition(Vector2.right * canvasSize.x, canvasSize, rb.position);
            return;
        }
        if (wall == Vector2.down) {
            rb.position = TeleportPosition(Vector2.zero, Vector2.right * canvasSize.x, rb.position);
            return;
        }
        if (wall == Vector2.up) {
            rb.position = TeleportPosition(Vector2.up * canvasSize.y, canvasSize, rb.position);
            return;
        }

    }

    public static Vector2 TeleportPosition(Vector2 WallPoint1, Vector2 WallPoint2, Vector2 objectPoint)
    {
    ...
    [System.Serializable] public class ObjectQueue {
    ...
    public static ObjectManager singltone { get; private set; }
    ...
    private void Awake()
    {
        ...
        s_CanvasRectTransformLink = canvasRectTransformLink;
...
public abstract class SpaceObject : MonoBehaviour, MenuScript.IResetable
{
